...
class ResNet18Classifier(nn.Module):
    def __init__(
        ...
        self.resnet18.fc1 = nn.Linear(
            in_features=self.resnet18.fc.in_features, out_features=2
        )
        self.resnet18.fc2 = nn.Linear(
            in_features=2, out_features=num_classes
        )
    ...
    def forward(self, input):
        ...
        x = torch.flatten(x, 1)
        x = self.drop(x)
        features = self.resnet18.fc1(x)

        x = self.drop(features)
        outputs = self.resnet18.fc2(x)
